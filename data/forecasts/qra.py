...
train_ger = ger_merged.loc['2020-01-01':'2020-07-02']
X_train_ger = train_ger[X_cols].to_numpy()
Y_train_ger = train_ger[Y_col].to_numpy()
test_ger = ger_merged.loc['2020-07-03': ]
...
qra_ger["y_true"] = ger_merged.loc['2020-07-03': ,'day_ahead_price_eur_mwh']

qra_ger = pd.DataFrame(qra_ger)
qra_ger.to_csv('data/qra/ger_qra_.csv')
...
if __name__ == '__main__': # without this, the QR_tester cannot run
    ...
    ger_results = QR_Tester(
        calibration_window=182*24, # MUCH more than the defautl of 7 is needed. 
        prediction_window=24,
        qr_model=qra_model,
        max_workers=10, # CAREFUL if you cloned this repo: if you're not sure your computer has this many workers, set this to 4
    ).fit_predict(X, y)

    # apparently it HAS to be pickled for the Christoffersen test to work 
    ger_results.to_pickle('data/qra/ger_qra.pkl')
...
train_es = es_merged.loc['2020-01-01':'2020-07-02']
X_train_es = train_es[X_cols].to_numpy()
Y_train_es = train_es[Y_col].to_numpy()
test_es= es_merged.loc['2020-07-03': ]
...
qra_es["y_true"] = es_merged.loc['2020-07-03': ,'day_ahead_price_eur_mwh']
qra_es = pd.DataFrame(qra_es)
qra_es.to_csv('data/qra/es_qra.csv')
...
if __name__ == '__main__': 
    ...
    # apparently it HAS to be pickled for the Christoffersen test to work 
    es_results.to_pickle('data/qra/es_qra.pkl')
